...
SELECT
  TO_CHAR(DATE_TRUNC('week', coalesce(cents_refunded_at, yrd_refunded_at)),'YYYY-MM-DD') AS week,
  (SUM(ABS(cents))/100)::FLOAT as dollars
FROM yerdle_line_item_bases
WHERE
  cents_refunded_at >= '2015-01-01' AND cents_refunded_at < TO_CHAR(DATE_TRUNC('week', CURRENT_DATE), 'YYYY-MM-DD')
