SELECT
  TO_CHAR(DATE_TRUNC('week', coalesce(closed_at, deleted_at, suspended_at)), 'YYYY-MM-DD') AS week,
  COUNT(id)
FROM yerdle_users
WHERE
  created_at >= '2015-01-01' AND created_at < TO_CHAR(DATE_TRUNC('week', CURRENT_DATE), 'YYYY-MM-DD')
  AND TO_CHAR(DATE_TRUNC('week', coalesce(closed_at, deleted_at, suspended_at)), 'YYYY-MM-DD') < CURRENT_DATE
