-- retained-ongoing (Joined 3+ months ago, active this month, AND active last month)
-- see retained_ongoing_users.sql
SELECT
...
FROM (
       ...
       FROM etl_dates AS d
         ...
         LEFT JOIN (
                     SELECT
                       ...
                       p0.month   AS month -- Join two mr_actives_vw tables with same data
                     FROM (
                            SELECT
                              user_id,
  ...
  JOIN yerdle_postings AS p ON a.user_id = p.poster_id AND a.day = DATE_TRUNC('day', p.created_at) AND
                               p.created_at >= TO_CHAR(CURRENT_DATE-1, 'YYYY-MM-01') AND p.created_at < CURRENT_DATE
GROUP BY 1
ORDER BY 1 DESC