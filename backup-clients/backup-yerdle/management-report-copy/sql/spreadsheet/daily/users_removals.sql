...
SELECT
  TO_CHAR(DATE_TRUNC('day', coalesce(closed_at, deleted_at, suspended_at)), 'YYYY-MM-DD') AS day,
  COUNT(id)
FROM yerdle_users
WHERE
  created_at >= '2015-01-01' AND created_at < CURRENT_DATE AND coalesce(closed_at, deleted_at, suspended_at) < CURRENT_DATE
