...
FROM (
  ...
  JOIN yerdle_postings AS p ON a.user_id = p.poster_id AND a.day = DATE_TRUNC('day', p.created_at) AND p.created_at >= '2015-01-01' AND p.created_at < TO_CHAR(DATE_TRUNC('month', CURRENT_DATE), 'YYYY-MM-DD')
GROUP BY 1
ORDER BY 1 DESC